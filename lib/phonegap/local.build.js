...
// standard after_prepare event emitted by cordova
cordova.on('after_prepare', function(data, done) {
    injectPhoneGapDotJS(data.paths[0], done);
});

// windows-phone specific build event that is emitted before the final packaging.
cordova.on('pre_package',function(data, done) {
    injectPhoneGapDotJS(data.wwwPath, done);
});

function injectPhoneGapDotJS(srcFolder, done) {
    ...
    // allow the other event hooks to proceed
    done = done || function() {};
    done();
}
