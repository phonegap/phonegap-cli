...
CreateCommand.prototype.execute = function(options, callback) {
    ...
    if (options['recipe']) {
        var gitUrl = require('../../recipes')[options['recipe']].repo
        // FIXME add an error here if we don't find a recipe
        cordova.config(options.path, {
            lib: {
                www: {
                    id: crypto.createHash('sha1').update(gitUrl).digest("hex"),
                    version: version,
                    uri: url,
                    link: false
                }
        ...
        cordovaCommand += ' --copy-from=' + gitUrl
    }
