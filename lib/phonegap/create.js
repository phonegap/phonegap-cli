...
CreateCommand.prototype.execute = function(options, callback) {
    ...
    /*!
     * Setup `options.config` with requested template.
     *
     * When a template was requested, we must augment the `config.config`
     * object to define the template to be used.
     *
     * Options:
     *
     *   - `configureTemplateCallback` {Function} is triggered on success.
     */

    function configureTemplate(configureTemplateCallback) {
        ...
        if (!declaresExternalTemplate(options)) {
            ...
            */
            ...
            // set options.config.lib to equal values for our template, pass this
            ...
            // when the system has a network connection, then we want to delete
            // the cached template and fetch the latest version.
            network.isOnline(function(online) {
                // if online and user requested a specific template (in other
                // words, we are not using the default hello-world template
                // that is set when no other template is defined).
                if (online && options.template) {
                    var templatePath = path.join(config.global.path, 'lib/www/', options.config.lib.www.id);
            ...
            // done (no templates to configure)
            configureTemplateCallback();
        }
    }

    // setup `options.config` based on any template requests.
