...
CreateCommand.prototype.execute = function(options, callback) {
    ...
    if (!declaresExternalTemplate(options)) {
        // default template
        template.id = 'com.phonegap.hello-world';
        template.version = 'master';
        template.url = 'https://github.com/phonegap/phonegap-app-hello-world/archive/' +
                       template.version +
                       '.tar.gz';
...
function getTemplateInfo(name) {
    ...
    try {
        ...
        template.version = 'master';
        template.id = crypto.createHash('sha1').update(template.url).digest('hex');
    }
    catch(e) {
