...
CreateCommand.prototype.execute = function(options, callback) {
    ...
    if (!declaresExternalTemplate(options)) {
        // default template
        recipe.id = 'com.phonegap.hello-world';
        recipe.version = 'master';
        recipe.url = 'https://github.com/phonegap/phonegap-app-hello-world/archive/' +
                     recipe.version +
                     '.tar.gz';

        // override with a user specified template
        if (options.recipe) {
            recipe = getRecipeInfo(options.recipe);
 ...
 *
 * An external template is a template that is not included in the recipe list.
 */
 ...
 * Get Recipe Info.
 *
 * Attempts to get the recipe information and generates the UUID used by
...
function getRecipeInfo(recipeName) {
    var recipes = require('../../package.json').recipes,
        recipe = {};

    try {
        recipe.url = recipes[recipeName].url;
        recipe.version = 'master';
        recipe.id = crypto.createHash('sha1').update(recipe.url).digest('hex');
    }
    catch(e) {
        recipe = new Error('the recipe "' + recipeName + '" was not found');
    }
