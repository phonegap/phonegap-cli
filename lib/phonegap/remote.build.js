...
var execute = function(options, callback) {
    ...
    self.remote.login({}, function(e, api) {
        ...
        var build = self.phonegapbuild.build(data, function(e, data) {
            ...
            self.emit('log', 'generating the QRCode...');
            ...
            qrcode.generate(url, function(qrcode) {
                self.emit('raw', qrcode);
                self.emit('log', 'install the app by scanning the QRCode');
                callback(null, data);
            });
        });

        build.on('compress', function() {
            self.emit('log', 'compressing the web app...');
        });

        build.on('upload', function() {
            self.emit('log', 'uploading to', 'build.phonegap.com'.underline + '...');
        });

        build.on('build', function() {
            self.emit('log', 'waiting for the', data.platforms[0], 'app to build...');
        });
    });
};
