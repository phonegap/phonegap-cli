...
LocalInstallCommand.prototype.execute = function(options, callback) {
    ...
    // install app
    if (options.device) {
        self.phonegap.emit('log', 'installing app onto ' + platform.human + ' device');
        cordova.run([ platform.local ], _callback);
    }
    else if (options.emulator) {
        self.phonegap.emit('log', 'installing app onto ' + platform.human + ' emulator');
        cordovaEmulate(self, platform);
        cordova.emulate([ platform.local ], _callback);
    }
    else {
        self.phonegap.emit('log', 'installing app onto ' + platform.human + ' device and falling back on emulator');
