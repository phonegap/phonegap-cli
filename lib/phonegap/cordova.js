...
CordovaCommand.prototype.run = function(options, callback) {
    ...
    if (isCustomCommand(options)) {
        ...
        if (options.cmd == 'cordova platform add browser@4.0.0') {
...
CordovaCommand.prototype.execute = function(options, callback) {
    ...
    // third try to find the cordova binary for Electron apps
    if (!fs.existsSync(path.join(binPath, 'cordova'))) {
    ...
    var cordovaCommandArgs = options.cmd.substring(command.length + 1);
    ...
    cordovaCommandArgs = (cordovaCommand + ' ' + cordovaCommandArgs).replace(/"/g, "");
    cordovaCommand = path.join("/Users/spindori/Documents/PhoneGap/phonegap-app-desktop/build/PhoneGap-darwin-x64/PhoneGap.app/Contents/Frameworks/PhoneGap Helper.app/Contents/MacOS/PhoneGap Helper");

    console.log('*** cordovaCommand: ', cordovaCommand);
    console.log('*** cordovaCommandArgs: ', cordovaCommandArgs);
    ...
    // var args = require('minimist')(cordovaCommand.split(' '));
    // console.log('*** args: ', args);

    // console.log(process.env);
    // shell out the command to cordova
    var spawn = require('child_process').spawn;
    var child = spawn(cordovaCommand, cordovaCommandArgs.split(' '), opts);

    // // cordovaCommand += ' ' + cordovaCommandArgs;
