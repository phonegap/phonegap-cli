...
CordovaCommand.prototype.execute = function(options, callback) {
    ...
    var cordovaExecError = function(code) {
        var e;
        if (code !== 0) {
    ...
    if (!process.env.ELECTRON_RUN_AS_NODE) {
        ...
        child = shell.exec(cordovaCommand, execOptions, function(code) {
            var error = cordovaExecError(code);
            callback(error);
        });
    } else {
        // run commands that come from Electron
        ...
        cordovaCommand = process.env.ELECTRON_HELPER_PATH;

        // shell out the command to cordova
        var spawn = require('child_process').spawn;
        child = spawn(cordovaCommand, cordovaCommandArgs.split(' '));
        ...
        child.on('error', function(e) {
            self.phonegap.emit('error', e.message);
        });
    }
