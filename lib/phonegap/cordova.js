...
CordovaCommand.prototype.execute = function(options, callback) {
    ...
    var binPath = path.join(__dirname, '..', '..', 'node_modules', '.bin'),
        command = options.cmd.split(' ')[0], // get the 'cordova' process name (it may change one day)
        cordovaCommand = path.join(binPath, command) + ' ' + options.cmd.substr(command.length + 1), // append args
