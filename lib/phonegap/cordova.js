...
CordovaCommand.prototype.run = function(options, callback) {
    ...
    self.addMissingPlatforms(options, function(e) {
        ...
        self.addPhoneGapJS(options, function(e) {
            ...
            // warn about phonegap.js deprecation
            self.addPhoneGapJSWarning(options, function(e) {
                if (e) {
                    return callback(e);
                }
...
CordovaCommand.prototype.execute = function(options, callback) {
    ...
    var child = shell.exec(cordovaCommand, execOptions, function(code, output) {
        ...
        if (code !== 0) {
            ...
            if (isCustomCommand(options)) {
                self.phonegap.emit('log', 'successfully executed',
                                          '\''+options.cmd+'\'');
            }
        }
        callback(e);
...
CordovaCommand.prototype.addPhoneGapJSWarning = function(options, callback) {
    ...
    if (match) {
        ...
        var projectRootPath = cordova.util.isCordova(),
            htmlGlob = path.join(projectRootPath, 'www', '*.html'),
            matchedFiles = shell.grep('phonegap.js', htmlGlob).trim();

        if (matchedFiles.length > 0) {
            self.phonegap.emit('warn', 'DEPRECATION: phonegap.js support will soon be removed.');
            self.phonegap.emit('warn', 'DEPRECATION: please replace \'phonegap.js\' references with \'cordova.js\'');
        }
    }
