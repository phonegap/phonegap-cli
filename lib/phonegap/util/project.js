...
/**
 * Get the path to the Cordova project.
 *
 * Options:
 *
 *   - [`projectPath`] {String} is optional path to check (default: cwd).
 *
 * Returns:
 *
 *   - {String} path to Cordova project or `undefined`
 */

module.exports.getProjectPath = function(projectPath) {
    // default to current working directory
    projectPath = projectPath || process.cwd();

    // is a cordova project?
    if (cdvutil.isCordova(projectPath)) {
        return projectPath;
    }
    // is root of file system?
    else if (projectPath === path.dirname(projectPath)) {
        return undefined;
    }
    // walk back one directory and try again
    else {
        projectPath = path.dirname(projectPath);
        return module.exports.getProjectPath(projectPath);
    }
};
...
checkPlatform = function (plat) {
    var detected = module.exports.listPlatforms();
